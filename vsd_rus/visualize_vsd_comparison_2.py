...
# Доля симуляции, отбрасываемая как переходный процесс.
# Согласовано с run_simulation.TAIL_FRAC = 0.75
STEADY_FRAC = 0.75

# Целевое число точек при прореживании рядов (для графиков).
N_SUBSAMPLE = 5000
...
def safe_savgol(data: np.ndarray, window: int = 101,
...
def subsample(data: dict, key: str, n_target: int = N_SUBSAMPLE
              ) -> Tuple[np.ndarray, np.ndarray]:
    """Прореживает ряд data[key] до ~n_target точек. Безопасен к NaN."""
...
def steady_mask(data: dict, frac: float = STEADY_FRAC) -> np.ndarray:
...
def print_statistical_summary(results: Dict[str, dict]) -> None:
    ...
    metrics = [
        ...
        ("O2_consumption",     "Потребление O₂ мозгом",  "у.е./с",     "{:.2f} ± {:.2f}", 1.0),
    ]
