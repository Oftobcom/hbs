...
class Heart4Chambers(OrganModel):
    """
    Четырёхкамерная модель сердца здорового человека.
    Клапаны работают нормально, дефект межжелудочковой перегородки (VSD) отсутствует.
    """
    def __init__(self,
                 hr=70,
    ...
    def get_derivatives(self, t, state, inputs):
        ...
        Q_vsd = 0.0  # здоровое сердце – нет дефекта
        ...
        dV_lv = Q_mitral - Q_aortic
        dV_ra = Q_sv_to_ra - Q_tricuspid
        dV_rv = Q_tricuspid - Q_pulmonary
